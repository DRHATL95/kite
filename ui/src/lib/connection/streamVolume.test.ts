...
describe("streamVolume", () => {
  it("caps the slider at 150%", () => {
    expect(VOLUME_MAX_PCT).toBe(150);
  });
  ...
  it("maps percent to a linear gain", () => {
    ...
    expect(pctToGain(150)).toBe(1.5);
  });

  it("clamps out-of-range input to [0, 1.5]", () => {
    expect(pctToGain(200)).toBe(1.5);
